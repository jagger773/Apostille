...
.controller 'ProductSelectController', ($scope, RequestService, $uibModalInstance, modalOptions, stockProductModalOptions, search, $uibModal) ->
    ...
    vm.gridOptions =
        ...
        columnDefs: [
            {name: 'name', displayName: 'Наименование', width: 250}
            {name: 'data.code', displayName: 'Код', visible: false}
            {name: 'data.barcode', displayName: 'Штрих код', visible: false}
            {name: 'product_type_value.data.name', displayName: 'Тип', visible: false}
            {name: 'category.name', displayName: 'Категория'}
            {name: 'unit.data.short_name', displayName: 'Ед. изм.'}
            {name: 'data.price', displayName: 'Цена (в сомах)'}
        ].reduce(
            (memo,item)->
        ...
        filter =
            ...
            filter:
                ...
                data:
                    ...
                    unit_id: vm.unit_id
        RequestService.post 'product.listing', filter
        .then (result)->
